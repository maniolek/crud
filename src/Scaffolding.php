...
use Vegas\Crud\Scaffolding\AdapterInterface;
use Vegas\Crud\Scaffolding\Exception\DeleteFailureException;
use Vegas\Crud\Scaffolding\Exception\InvalidFormException;
...
class Scaffolding implements ScaffoldingInterface
{
    ...
    private function processForm($values)
    {
        ...
        if ($messages->count() > 0) {
            $messages->rewind();
            throw new InvalidFormException($messages->current()->getMessage());
        }
        throw new InvalidFormException($form->getMessages());
    }
